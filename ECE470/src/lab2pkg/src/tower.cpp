...
/* move_to_point
    ...
    input: array containing one waypoint
    output: 0 if successful, 1 if unsuccessful or input invalid */
int move_to_point(double waypoint[]);
...
double arr12[]={169.94*PI/180,-85.7*PI/180,117.82*PI/180,-122.04*PI/180,-89.51*PI/180,78.83*PI/180};
double arr13[]={199.87*PI/180,-71.14*PI/180,95.07*PI/180,-114.27*PI/180,-89.42*PI/180,108.79*PI/180};
...
int move_to_point(double waypoint[])
{
    ...
    std::vector<double> QH (waypoint,waypoint+sizeof(waypoint) / sizeof(waypoint[0]));
