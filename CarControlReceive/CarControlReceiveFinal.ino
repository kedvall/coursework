...
void loop() {
  ...
  if (driver.recv(buf, &buflen)) //Check if RF Receiver if receiving data (non-blocking)
  {
    ...
    //Print to serial console for debugging (Uncomment to check raw values being received)
    /*
    Serial.print("Ping: ");
    ...
    */
    ...
    //Turn on built in LED when joystick button is pressed (safe mode indicator)
    if (!dataPacket.btnState) //Check button state
      digitalWrite(LED_PIN, HIGH); //Toggle built in LED
    else
