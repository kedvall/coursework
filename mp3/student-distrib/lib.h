...
// KEV-KEV'S COLORED ASCII FUNCTIONS ;D
extern int32_t colored_puts(int8_t* s, int x_coord, int y_coord, uint8_t backg_color, uint8_t text_color);
extern void colored_putc(int8_t s, int x_coord, int y_coord, uint8_t backg_color, uint8_t text_color);
extern int8_t save_c(int x_coord, int y_coord);

/* GANK ADDED FUNCTIONS :D */
extern void clearLine(int firstLineOverflowed);
extern uint32_t buffered_puts(uint8_t* s, int nbytes);
extern void buffered_putc(uint8_t c);
extern void clearShell();
extern void update_cursor(int8_t use_active);
extern void scrollUp(int8_t use_active);
